...
const AlphabetCard: React.FC<AlphabetCardProps> = ({ alphabet }) => {
  ...
  const handleSpeak = () => {
    if (alphabet.audioSrc) {
      ...
      // Vite handles assets in 'public' folder by serving them from root.
      // So, if audioSrc is 'audio/alphabets/a.mp3', it's relative to public root.
      // The base path from vite.config.ts will be prepended automatically by the browser
      // when resolving this relative path if it's used in e.g. new Audio(alphabet.audioSrc).
